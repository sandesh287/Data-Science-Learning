...
model = models.Sequential([
  ...
  # This is LSTM layer with 32 units which processes the sequence data. LSTMs are designed to capture sequential dependencies in the data, making them well suited for text
  layers.LSTM(32),
  # This is the fully connected output layer with 1 unit and a sigmoid activation function. Here, "1", parameter represents a single output neuron predicting the probability of a positive or negative sentiment; Sigmoid activation function outputs a probability between 0 and 1 suitable for binary classification.
...
# With LSTM, we got a better accuracy for the same dataset for IMDb, than simple RNN model.