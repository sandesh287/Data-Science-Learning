...
# Example dataset: English-to-French sentences
english_sentences = ["hello", "how are you", "good morning", "thank you", "good night"]
french_sentences = ["bonjour", "comment ca va", "bonne matin", "merci", "bonne nuit"]
...
# Create max length
max_len_english = max(len(sentence.split()) for sentence in english_sentences) + 2
max_len_french = max(len(sentence.split()) for sentence in french_sentences) + 2


# Create sample dataset
english_data = tokenize(english_sentences, english_vocab, max_len_english)
french_data = tokenize(french_sentences, french_vocab, max_len_french)
...
# Create dataset element and DataLoader
dataset = TranslationDataset(english_data, french_data)
dataloader = DataLoader(dataset, batch_size=2, shuffle=True)



# Define Seq2Seq model components
...
translation = translate_sentence(model, sentence, english_vocab, french_vocab, max_len_french, device)
